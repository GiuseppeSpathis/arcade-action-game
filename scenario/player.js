...
export class PlayerController {
  ...
  executeJump(pressedKeys) {
    ...
    const impulse = this.constants.PLAYER.MAX_SPEED *
  ...
  isMovingLeft(pressedKeys) {
    return this.constants.INPUT.MOVE_LEFT_KEYS.some((code) => pressedKeys.has(code));
  }

  isMovingRight(pressedKeys) {
    return this.constants.INPUT.MOVE_RIGHT_KEYS.some((code) => pressedKeys.has(code));
  }
  ...
  update(pressedKeys) {
    ...
    if (this.state.vy > 0) {
      if (
        ...
        nextY =
          Math.floor(
            (this.state.y + this.state.height + this.state.vy - this.mapOffsetY) /
              this.tileSize
          ) *
            this.tileSize +
          this.mapOffsetY -
