...
// NEW: Import Firebase controller helpers
import {
  initFirebase,
  createGameSession,
...
async function initializeGame() {
  ...
  // --- NEW: Firebase Setup ---
  let remoteInputsState = {}; // Stores the last state to detect changes (for jump)
  let firebaseUnsubscribe;

  if (playerCount > 1) {
    const { error } = await initFirebase();
    if (error) {
      // Show error to user (e.g., in the room code display)
      roomCodeText.textContent = "ERROR";
      roomCodeDisplay.style.borderColor = "#E63946";
      roomCodeDisplay.querySelector(".room-code-label").textContent = "FIREBASE:";
      roomCodeDisplay.hidden = false;
    } else {
      try {
        const roomCode = await createGameSession(playerCount);
        if (roomCode) {
          roomCodeText.textContent = roomCode;
          roomCodeDisplay.hidden = false;

          // Start listening for inputs
          firebaseUnsubscribe = listenForRemoteInputs(roomCode, (newInputs) => {
            // newInputs looks like { p2: { l, r, j, s }, p3: ... }
            ...
            playerKeys.forEach((key, index) => {
              ...
              if (!newInputs[key] || !players[playerIndex]) {
                return;
              }

              const pInputs = newInputs[key];
              ...
              // Shoot is a state. Map 's' to the first shoot key (e.g., "Up").
              // The phone controller doesn't support directional shooting.
              pInputs.s
                ? pressedKeys.add(pConfig.shoot[0])
                : pressedKeys.delete(pConfig.shoot[0]);

              // Jump is an event. Trigger on rising edge (false -> true).
        ...
        console.error("Failed to create/listen to Firebase session:", e);
        roomCodeText.textContent = "ERROR";
        roomCodeDisplay.style.borderColor = "#E63946";
        roomCodeDisplay.querySelector(".room-code-label").textContent = "SESSION:";
        roomCodeDisplay.hidden = false;
      }
    }
  }
  ...
  function triggerGameOver() {
    ...
    // NEW: Stop listening to Firebase
  ...
  // NEW: Unsubscribe from Firebase if the window is closed
